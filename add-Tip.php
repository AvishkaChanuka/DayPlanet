...
<head>
    ...
    <title>Add Tip - Day Planet</title>
...
<body>
    <div class="container">
        <h1 class="mb-4 text-center text-success">Write a Tip</h1>

        <form id="addTipForm" enctype="multipart/form-data" method ="POST">
            <div class="mb-3">
                <label for="tipTitle" class="form-label">Tip Title</label>
                <input type="text" class="form-control" id="tipTitle" placeholder="Enter the tip title" required name="TipTitle">
            </div>
            <div class="mb-3">
                <label for="tipCategory" class="form-label">Category</label>
                <select class="form-select" id="tipCategory" required name="TipSDG">
            ...
            <div class="mb-3">
                <label for="tipContent" class="form-label">Content</label>
                <textarea class="form-control" id="tipContent" rows="10" placeholder="Write your tip here..." required name="TipContent"></textarea>
            </div>
            <button type="submit" class="btn btn-success w-100" name="AddTip">Submit Tip</button>
        </form>
    </div>
    ...
    <?php
        ...
        if(isset($_POST["AddTip"])){
            ...
            $query = "INSERT INTO tips (user_id, title, body, sdg_goal)
                        VALUES ($user, '".$tipTitle."', '".$TipContent."', '".$tipSDG."');";
          
            if(Create($query)){
                ShowSuccess("Tip Posted Successfully!!!");
            }
            else{
